...
var pascalize = (str) => _shared.capitalize.call(void 0, _shared.camelize.call(void 0, str));
var isNotNil = (value) => value != null;

// src/core/parseSFC.ts
var ELEMENT = 1;
var DIRECTIVE = 7;
var SIMPLE_EXPRESSION = 4;
var multilineCommentsRE = /\/\*\s(.|[\r\n])*?\*\//gm;
var singlelineCommentsRE = /\/\/\s.*/g;
var BUILD_IN_DIRECTIVES_WIRH_ARGUMENT = new Set([
  "on",
  "bind",
  "slot"
]);
var BUILD_IN_DIRECTIVES = new Set([
  "if",
  "else",
  "else-if",
  "for",
...
function maybeHasDirectiveArgument(nameWithPrefixV) {
...
function parseDirective(comp, attr, body) {
  const elementNode = _compilercore.baseCompile.call(void 0, `<${comp} ${attr}="${body}" />`).ast.children[0];
  if ((elementNode == null ? void 0 : elementNode.type) !== ELEMENT)
    return void 0;
  const directiveNode = elementNode.props[0];
  if ((directiveNode == null ? void 0 : directiveNode.type) !== DIRECTIVE)
    return void 0;
  const { arg, modifiers, name } = directiveNode;
  const argExpression = (arg == null ? void 0 : arg.type) !== SIMPLE_EXPRESSION ? void 0 : arg.isStatic ? JSON.stringify(arg.content) : arg.content;
  return {
    argExpression,
    modifiers,
    name
...
function parseSFC(code, id, options) {
  ...
  function handleTemplateContent(name, attributes) {
    ...
    Object.entries(attributes).forEach(([key, value]) => {
      ...
      if (["v-", "@[", ":["].some((prefix) => key.startsWith(prefix)) && maybeHasDirectiveArgument(key)) {
        const parsedDirective = parseDirective(name, key, value);
        if (parsedDirective && !BUILD_IN_DIRECTIVES.has(parsedDirective.name))
          directives.add(_shared.camelize.call(void 0, parsedDirective.name));
        if (parsedDirective == null ? void 0 : parsedDirective.argExpression)
...
function transformScriptSetup(sfc, options) {
  ...
  const directiveDeclaration = Array.from(template.directives).map((directive) => {
    const identifier = declarationArray.find((declaration) => declaration === `v${_shared.capitalize.call(void 0, directive)}`);
  ...
  if (directiveDeclaration.length) {
    hasBody = true;
    const directivesObject = _core.types.objectExpression(directiveDeclaration.map(({ directive, identifier }) => _core.types.objectProperty(_core.types.identifier(directive), _core.types.identifier(identifier), false, false)));
    ast.body.push(_core.types.expressionStatement(_core.types.assignmentExpression("=", _core.types.memberExpression(__sfc, _core.types.identifier("directives")), _core.types.callExpression(_core.types.memberExpression(_core.types.identifier("Object"), _core.types.identifier("assign")), [
      directivesObject,
      _core.types.memberExpression(__sfc, _core.types.identifier("directives"))
    ]))));
  }
