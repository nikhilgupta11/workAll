...
const useAddressesFactory = (factoryParams) => function useAddresses() {
    const loading = core.sharedRef(false, 'useAddresses-loading');
    const addresses = core.sharedRef(null, 'useAddresses-shipping');
    const error = core.sharedRef({
        load: null,
        save: null,
        remove: null,
        update: null,
    ...
    const _factoryParams = core.configureFactoryParams(factoryParams);
    const load = async (loadParams) => {
        core.Logger.debug('useAddresses.load');
        try {
            loading.value = true;
            ...
            core.Logger.error('useAddresses/load', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const save = async (saveParams) => {
        core.Logger.debug('useAddresses.save');
        try {
            loading.value = true;
            ...
            core.Logger.error('useAddresses/save', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const update = async (updateParams) => {
        core.Logger.debug('useAddresses.update');
        try {
            loading.value = true;
            ...
            core.Logger.error('useAddresses.delete', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const remove = async (removeParams) => {
        core.Logger.debug('useAddresses.remove');
        try {
            loading.value = true;
            ...
            core.Logger.error('useAddresses.remove', err);
        }
        finally {
            loading.value = false;
        }
    ...
    return {
        addresses: compositionApi.computed(() => addresses.value),
        error: compositionApi.computed(() => error.value),
        load,
        loading: compositionApi.computed(() => loading.value),
        remove,
        save,
        update,
    };
...
const factoryParams$t = {
    load: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento] load user addresses');
    ...
    update: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] update user addresses', params);
...
const useCartFactory = (factoryParams) => function useCart() {
    const loading = core.sharedRef(false, 'useCart-loading');
    const cart = core.sharedRef(null, 'useCart-cart');
    const totalQuantity = core.sharedRef(0, 'useCart-totalQuantity');
    const error = core.sharedRef({
        addItem: null,
        removeItem: null,
        updateItemQty: null,
    ...
    const setCart = (newCart) => {
        cart.value = newCart;
        core.Logger.debug('useCartFactory.setCart', newCart);
    };
    const addItem = async ({ product, quantity, customQuery, }) => {
        core.Logger.debug('useCart.addItem', {
            product,
            quantity,
        });
        try {
            ...
            core.Logger.error('useCart/addItem', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const removeItem = async ({ product, customQuery, }) => {
        core.Logger.debug('useCart.removeItem', { product });
        try {
            loading.value = true;
            ...
            core.Logger.error('useCart/removeItem', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const updateItemQty = async ({ product, quantity, customQuery, }) => {
        core.Logger.debug('useCart.updateItemQty', {
            product,
            quantity,
        });
        if (quantity && quantity > 0) {
            try {
                ...
                core.Logger.error('useCart/updateItemQty', err);
            }
            finally {
                loading.value = false;
            }
    ...
    const load = async ({ customQuery } = { customQuery: undefined }) => {
        var _a;
        core.Logger.debug('useCart.load');
        try {
            loading.value = true;
            ...
            error.value.load = err;
            core.Logger.error('useCart/load', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const loadTotalQty = async ({ customQuery } = { customQuery: undefined }) => {
        core.Logger.debug('useCart.loadTotalQty');
        try {
            loading.value = true;
            ...
            core.Logger.error('useCart/loadTotalQty', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const clear = async () => {
        core.Logger.debug('useCart.clear');
        try {
            loading.value = true;
            ...
            core.Logger.error('useCart/clear', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const applyCoupon = async ({ couponCode, customQuery, }) => {
        core.Logger.debug('useCart.applyCoupon');
        try {
            loading.value = true;
            ...
            core.Logger.error('useCart/applyCoupon', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const removeCoupon = async ({ couponCode, customQuery, }) => {
        core.Logger.debug('useCart.removeCoupon');
        try {
            loading.value = true;
            ...
            core.Logger.error('useCart/removeCoupon', err);
        }
        finally {
            loading.value = false;
        }
    ...
    return {
        ...
        setCart,
        cart: compositionApi.computed(() => cart.value),
        totalQuantity,
        isInCart,
        addItem,
        load,
...
const factoryParams$s = {
    load: async (context, params) => {
        ...
        core.Logger.debug('[Magento Storefront]: Loading Cart');
        ...
        const createRealCart = async () => {
            core.Logger.debug('[Magento Storefront]: useCart.load.createNewCart');
            apiState.setCartId();
        ...
        const getCartData = async (id) => {
            core.Logger.debug('[Magento Storefront]: useCart.load.getCartData ID->', id);
        ...
        if (customerToken) {
            try {
                ...
                core.Logger.debug('[Result]:', { data, errors });
    ...
    addItem: async (context, { product, quantity, currentCart, customQuery, }) => {
        var _a, _b;
        core.Logger.debug('[Magento]: Add item to cart', {
            product,
            quantity,
            currentCart,
        });
        ...
        try {
            ...
            switch (product.__typename) {
                case 'SimpleProduct':
                    ...
                    core.Logger.debug('[Result]:', { data: simpleProduct });
                ...
                case 'ConfigurableProduct':
                    ...
                    core.Logger.debug('[Result]:', { data: configurableProduct });
                    // eslint-disable-next-line consistent-return
                ...
                case 'BundleProduct':
                    ...
                    core.Logger.debug('[Result]:', { data: bundleProduct });
                    // eslint-disable-next-line consistent-return
                ...
                case 'DownloadableProduct':
                    ...
                    core.Logger.debug('[Result DownloadableProduct]:', { data: downloadableProduct });
                    // eslint-disable-next-line consistent-return
                ...
                case 'VirtualProduct':
                    ...
                    core.Logger.debug('[Result VirtualProduct]:', { data: virtualProduct });
                    // eslint-disable-next-line consistent-return
    ...
    removeItem: async (context, { currentCart, product, }) => {
        core.Logger.debug('[Magento]: Remove item from cart', {
            product,
            currentCart,
        });
    ...
    updateItemQty: async (context, { currentCart, product, quantity, }) => {
        core.Logger.debug('[Magento]: Update product quantity on cart', {
            product,
            quantity,
            currentCart,
        });
    ...
    applyCoupon: async (context, { currentCart, couponCode, }) => {
        core.Logger.debug('[Magento]: Apply coupon on cart', {
            couponCode,
            currentCart,
        });
    ...
    removeCoupon: async (context, { currentCart }) => {
        core.Logger.debug('[Magento]: Remove coupon from cart', { currentCart });
...
const factoryParams$r = {
    ...
    load: async (context, { customQuery }) => {
        var _a, _b, _c, _d, _e;
        core.Logger.debug('[Magento] loadShippingProvider', { customQuery });
    ...
    save: async (context, params) => {
        core.Logger.debug('[Magento] saveShippingProvider', { params });
...
var useShippingProvider = core.useShippingProviderFactory(factoryParams$r);
...
const factoryParams$q = {
    ...
    load: async (context, { customQuery }) => {
        var _a, _b, _c, _d, _e, _f;
        core.Logger.debug('[Magento] loadBilling');
    ...
    save: async (context, params) => {
        var _a, _b, _c;
        core.Logger.debug('[Magento] setBillingAddress');
        ...
        core.Logger.debug('[Magento]: Defining the shipping method as:', JSON.stringify(shippingMethod, null, 2));
...
var index$f = core.useBillingFactory(factoryParams$q);

const factoryParams$p = {
    categorySearch: async (context, params) => {
        core.Logger.debug('[Magento]: List available categories', { params });
        const { data } = await context.$magento.api.categoryList(params);
        core.Logger.debug('[Result]:', { data });
        return data.categories.items;
    },
};
var index$e = core.useCategoryFactory(factoryParams$p);

function useCategorySearchFactory(factoryParams) {
    return function useCategorySearch(id = '') {
        const ssrKey = id || 'useCategorySearch';
        // @ts-ignore
        const result = core.sharedRef([], `${ssrKey}-result`);
        const loading = core.sharedRef(false, `${ssrKey}-loading`);
        const error = core.sharedRef({
            search: null,
        }, `${ssrKey}-error`);
        // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
        ...
        const search = async (params) => {
            core.Logger.debug(`useCategorySearch/${ssrKey}/search`);
            try {
                loading.value = true;
                ...
                core.Logger.error(`useCategorySearch/${ssrKey}/search`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        return {
            search,
            result: compositionApi.computed(() => result.value),
            loading: compositionApi.computed(() => loading.value),
            error: compositionApi.computed(() => error.value),
        };
    };
...
const factoryParams$o = {
    search: async (context, params) => {
        core.Logger.debug('[Magento]: Search for category using', { params });
...
function useConfigFactory(factoryParams) {
    return function useConfig(cacheId = '') {
        ...
        // @ts-ignore
        const config = core.sharedRef({}, `${ssrKey}-config`);
        const loading = core.sharedRef(false, `${ssrKey}-loading`);
        // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
        const _factoryParams = core.configureFactoryParams(factoryParams);
        const loadConfig = async (params) => {
            core.Logger.debug(`useConfig/${ssrKey}/loadConfig`);
            loading.value = true;
            try {
        ...
        return {
            loadConfig,
            config: compositionApi.computed(() => config.value),
            loading: compositionApi.computed(() => loading.value),
        };
    };
}
...
const factoryParams$n = {
    loadConfig: async (context) => {
        ...
        core.Logger.debug('[Magento] useConfig:loadConfig data', { data });
...
function useContentFactory(factoryParams) {
    return function useContent(ssrKey = 'useConfigFactory') {
        // @ts-ignore
        const page = core.sharedRef({}, `useContent-content-${ssrKey}`);
        const errors = core.sharedRef({
            content: null,
            blocks: null,
        }, 'useContent-error');
        const blocks = core.sharedRef([], `useContent-blocks-${ssrKey}`);
        const loading = core.sharedRef(false, `useContent-loading-${ssrKey}`);
        // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
        const _factoryParams = core.configureFactoryParams(factoryParams);
        const loadContent = async (params) => {
            core.Logger.debug(`useContent/${ssrKey}/loadPage`);
            loading.value = true;
            try {
        ...
        const loadBlocks = async (params) => {
            core.Logger.debug(`useContent/${ssrKey}/loadBlocks`);
            loading.value = true;
            try {
        ...
        return {
            ...
            loadBlocks,
            loading: compositionApi.computed(() => loading.value),
            page: compositionApi.computed(() => page.value),
            blocks: compositionApi.computed(() => blocks.value),
            error: compositionApi.computed(() => errors.value),
        };
    };
}
...
const factoryParams$m = {
    loadContent: async (context, params) => {
        core.Logger.debug('[Magento]: Load CMS Page content', { params });
        const { data } = await context.$magento.api.cmsPage(params.identifier);
        core.Logger.debug('[Result]:', { data });
        return data.cmsPage;
    },
    loadBlocks: async (context, params) => {
        core.Logger.debug('[Magento]: Load CMS Blocks content', { params });
...
function useCountrySearchFactory(factoryParams) {
    return function useCountrySearch(cacheId = '') {
        ...
        const countries = core.sharedRef([], `${ssrKey}-countries`);
        // @ts-ignore
        const country = core.sharedRef({}, `${ssrKey}-country`);
        const loading = core.sharedRef(false, `${ssrKey}-loading`);
        const error = core.sharedRef({
            load: null,
            search: null,
        ...
        const load = async (params) => {
            core.Logger.debug(`useCountrySearch/${ssrKey}/load`);
            try {
                loading.value = true;
                ...
                error.value.load = err;
                core.Logger.error(`useCountrySearch/${ssrKey}/load`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        // eslint-disable-next-line consistent-return
        const search = async (params) => {
            core.Logger.debug(`useCountrySearch/${ssrKey}/search`);
            try {
                loading.value = true;
                const data = await _factoryParams.search(params);
                country.value = data;
                ...
                error.value.search = err;
                core.Logger.error(`useCountrySearch/${ssrKey}/search`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        return {
            load,
            search,
            countries: compositionApi.computed(() => countries.value),
            country: compositionApi.computed(() => country.value),
            loading: compositionApi.computed(() => loading.value),
            error: compositionApi.computed(() => error.value),
        };
    };
...
const factoryParams$l = {
    load: async (context, _params) => {
        core.Logger.debug('[Magento]: Load available countries on store');
        const { data } = await context.$magento.api.countries();
        core.Logger.debug('[Result]:', { data });
        return data.countries;
    },
    search: async (context, params) => {
        core.Logger.debug('[Magento]: Search country information based on', { params });
...
function useCurrencyFactory(factoryParams) {
    return function useCurrency(cacheId = '') {
        ...
        // @ts-ignore
        const currencies = core.sharedRef([], `${ssrKey}-currencies`);
        const loading = core.sharedRef(false, `${ssrKey}-loading`);
        // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
        const _factoryParams = core.configureFactoryParams(factoryParams);
        const load = async (params) => {
            core.Logger.debug(`useCurrency/${ssrKey}/load`);
            loading.value = true;
            try {
        ...
        const change = async (params) => {
            core.Logger.debug(`useCurrency/${ssrKey}/change`);
            loading.value = true;
            try {
        ...
        return {
            load,
            change,
            currencies: compositionApi.computed(() => currencies.value),
            loading: compositionApi.computed(() => loading.value),
        };
    };
}
...
const useCustomMutationFactory = (factoryParams) => function useCustomMutation(ssrKey = 'useCustomMutation') {
    // @ts-ignore
    const mutationString = core.sharedRef('', `${ssrKey}-mutationString`);
    const mutationStringComputed = compositionApi.computed(() => mutationString.value);
    // @ts-ignore
    const result = core.sharedRef({}, `${ssrKey}-result`);
    const loading = core.sharedRef(false, `${ssrKey}-loading`);
    const error = core.sharedRef({
    ...
    const mutation = async ({ variables, fetchPolicy, }) => {
        core.Logger.debug(`useCustomMutation/${ssrKey}/mutation`);
        loading.value = true;
        try {
            ...
            error.value.search = err;
            core.Logger.error(`useCustomMutation/${ssrKey}/mutation`, err);
        }
        finally {
            loading.value = false;
        }
...
var index$c = useCustomMutationFactory({
    mutation: async (context, { mutation, variables, fetchPolicy, }) => {
        core.Logger.debug('[Magento] Custom API Mutation', { variables });
        const result = await context.$magento.api.customMutation({
            mutation,
            mutationVariables: variables,
            fetchPolicy,
        });
        core.Logger.debug('[Custom Mutation -> Result]:', result);
        return result;
    },
});
...
const useCustomQueryFactory = (factoryParams) => function useCustomQuery(ssrKey = 'useCustomQuery') {
    // @ts-ignore
    const queryString = core.sharedRef('', `${ssrKey}-queryString`);
    const queryStringComputed = compositionApi.computed(() => queryString.value);
    // @ts-ignore
    const result = core.sharedRef({}, `${ssrKey}-result`);
    const loading = core.sharedRef(false, `${ssrKey}-loading`);
    const error = core.sharedRef({
    ...
    const query = async ({ variables, fetchPolicy, }) => {
        core.Logger.debug(`useCustomQuery/${ssrKey}/query`);
        loading.value = true;
        try {
            ...
            error.value.search = err;
            core.Logger.error(`useCustomQuery/${ssrKey}/query`, err);
        }
        finally {
            loading.value = false;
        }
...
var index$b = useCustomQueryFactory({
    query: async (context, { query, variables, fetchPolicy, }) => {
        core.Logger.debug('[Magento] Custom API Query', { variables });
        const result = await context.$magento.api.customQuery({
            query,
            queryVariables: variables,
            fetchPolicy,
        });
        core.Logger.debug('[Custom Query -> Result]:', result);
        return result;
    },
});
...
const useExternalCheckoutFactory = (factoryParams) => {
    function useExternalCheckout(ssrKey = 'useExternalCheckout') {
        ...
        const initializeCheckout = async (params) => {
            core.Logger.debug(`useExternalCheckout/${ssrKey}/initializeCheckout`);
            loading.value = true;
            try {
                ...
                error.value.search = err;
                core.Logger.error(`useExternalCheckout/${ssrKey}/initializeCheckout`, err);
            }
            finally {
                loading.value = false;
            }
...
const factoryParams$i = {
    ...
    search: async (context, params) => {
        var _a, _b, _c;
        core.Logger.debug('[Magento] Load product facets', { params });
...
var index$9 = core.useFacetFactory(factoryParams$i);

const factoryParams$h = {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    resetPassword: async (context, params) => {
        core.Logger.debug('[Magento]: Reset user password', { params });
    ...
    setNewPassword: async (context, params) => {
        core.Logger.debug('[Magento]: Define new user password', { params });
...
const useForgotPassword = core.useForgotPasswordFactory(factoryParams$h);
...
function useGetShippingMethodsFactory(factoryParams) {
    return function useGetShippingMethods(id = '') {
        // @ts-ignore
        const state = core.sharedRef(null, 'UseGetShippingMethods-response');
        ...
        // eslint-disable-next-line consistent-return
        const load = async (params) => {
            core.Logger.debug(`useGetShippingMethods/${ssrKey}/load`, { params });
            try {
                loading.value = true;
                const data = await _factoryParams.load(params);
                state.value = data;
                ...
                error.value.search = err;
                core.Logger.error(`useGetShippingMethods/${ssrKey}/load`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const setState = (newState) => {
            ...
            core.Logger.debug('useGetShippingMethods.setState', newState);
        };
        return {
            state,
            setState,
            load,
            result: compositionApi.computed(() => state.value),
            loading: compositionApi.computed(() => loading.value),
            error: compositionApi.computed(() => error.value),
        };
    };
...
const factoryParams$g = {
    load: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento]: Load shipping methods', { params });
...
const useGuestUserFactory = (factoryParams) => function useGuestUser() {
    ...
    const guestUser = core.sharedRef(null, 'useGuestUser-user');
    const loading = core.sharedRef(false, 'useGuestUser-loading');
    // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
    const _factoryParams = core.configureFactoryParams(factoryParams);
    const error = core.sharedRef(errorsFactory(), 'useGuestUser-error');
    const setGuestUser = (newUser) => {
        guestUser.value = newUser;
        core.Logger.debug('useGuestUserFactory.setGuestUser', newUser);
    };
    const resetErrorValue = () => {
        error.value = errorsFactory();
    };
    const attachToCart = async (params) => {
        core.Logger.debug('useGuestUserFactory.attachToCart', { params });
        resetErrorValue();
        try {
            loading.value = true;
            ...
            core.Logger.error('useGuestUser/attachToCart', err);
        }
        finally {
            loading.value = false;
        }
    ...
    return {
        setGuestUser,
        attachToCart,
        guestUser: compositionApi.computed(() => guestUser.value),
        loading: compositionApi.computed(() => loading.value),
        error: compositionApi.computed(() => error.value),
    };
};
...
const factoryParams$f = {
    ...
    attachToCart: async (context, params) => {
        core.Logger.debug('[Magento]: Attach guest cart to user');
...
const factoryParams$e = {
    ...
    make: async (context, params) => {
        core.Logger.debug('[Magento] Make Order', { params });
        const { id } = context.cart.cart.value;
...
const useMakeOrder = core.useMakeOrderFactory(factoryParams$e);
...
const factoryParams$d = {
    ...
    load: async (context) => {
        var _a;
        core.Logger.debug('[Magento] Load user information');
        const apiState = context.$magento.config.state;
    ...
    updateUser: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] Update user information', { params });
    ...
    logIn: async (context, params) => {
        core.Logger.debug('[Magento] Authenticate user');
        const apiState = context.$magento.config.state;
    ...
    changePassword: async (context, params) => {
        core.Logger.debug('[Magento] changing user password');
...
var useUser = core.useUserFactory(factoryParams$d);

const useNewsletterFactory = (factoryParams) => function useNewsletter() {
    const errorsFactory = () => ({
        updateSubscription: null,
    });
    const loading = core.sharedRef(false, 'useNewsletter-loading');
    // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
    const _factoryParams = core.configureFactoryParams(factoryParams);
    const error = core.sharedRef(errorsFactory(), 'useNewsletter-error');
    const resetErrorValue = () => {
        error.value = errorsFactory();
    };
    const updateSubscription = async (params) => {
        core.Logger.debug('useNewsletterFactory.updateSubscription', params);
        resetErrorValue();
        try {
            loading.value = true;
            ...
            core.Logger.error('useNewsletter/updateSubscription', err);
        }
        finally {
            loading.value = false;
        }
...
const factoryParams$c = {
    ...
    updateSubscription: async (context, params) => {
        core.Logger.debug('[Magento]: Update user newsletter subscription', { params });
...
const usePaymentProviderFactory = (factoryParams) => function usePaymentProvider() {
    const loading = core.sharedRef(false, 'usePaymentProvider-loading');
    const state = core.sharedRef(null, 'usePaymentProvider-response');
    // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
    const _factoryParams = core.configureFactoryParams(factoryParams);
    const error = core.sharedRef({
        load: null,
    ...
    const setState = (newState) => {
        state.value = newState;
        core.Logger.debug('usePaymentProvider.setState', newState);
    };
    const save = async (params) => {
        core.Logger.debug('usePaymentProvider.save');
        try {
            loading.value = true;
            ...
            error.value.save = err;
            core.Logger.error('usePaymentProvider/save', err);
        }
        finally {
            loading.value = false;
        }
    ...
    const load = async (params) => {
        core.Logger.debug('usePaymentProvider.load');
        try {
            loading.value = true;
            ...
            error.value.load = err;
            core.Logger.error('usePaymentProvider/load', err);
        }
        finally {
            loading.value = false;
        }
...
const factoryParams$b = {
    ...
    load: async (context, { customQuery }) => {
        core.Logger.debug('[Magento] loadPaymentProvider', { customQuery });
    ...
    save: async (context, params) => {
        core.Logger.debug('[Magento] savePaymentProvider', { params });
...
const factoryParams$a = {
    productsSearch: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento]: Load product based on ', { params });
        ...
        switch (queryType) {
            case "DETAIL" /* Detail */:
                ...
                core.Logger.debug('[Result]:', { data: productDetailsResults });
            ...
            default:
                ...
                core.Logger.debug('[Result]:', { data: productListResults });
                return (_b = productListResults.data) === null || _b === void 0 ? void 0 : _b.products;
        }
    },
};
const useProduct = core.useProductFactory(factoryParams$a);
var useProduct$1 = useProduct;

function useRelatedProductsFactory(factoryParams) {
    return function useRelatedProduct(id) {
        const products = core.sharedRef([], `useRelatedProducts-products-${id}`);
        const loading = core.sharedRef(false, `useRelatedProducts-loading-${id}`);
        const error = core.sharedRef({
            search: null,
        ...
        const search = async (searchParams) => {
            core.Logger.debug(`useRelatedProducts/${id}/search`, searchParams);
            try {
                loading.value = true;
                ...
                error.value.search = err;
                core.Logger.error(`useRelatedProducts/${id}/search`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        return {
            search,
            products: compositionApi.computed(() => products.value),
            loading: compositionApi.computed(() => loading.value),
            error: compositionApi.computed(() => error.value),
        };
    };
...
const factoryParams$9 = {
    productsSearch: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento] Load related products based on ', { params });
...
function useReviewFactory(factoryParams) {
    return function useReview(id) {
        const reviews = core.sharedRef([], `useReviews-reviews-${id}`);
        const metadatas = core.sharedRef([], `useReviews-metadata-${id}`);
        const loading = core.sharedRef(false, `useReviews-loading-${id}`);
        const error = core.sharedRef({
            search: null,
            addReview: null,
        ...
        const _factoryParams = core.configureFactoryParams(factoryParams);
        const search = async (searchParams) => {
            core.Logger.debug(`useReview/${id}/search`, searchParams);
            try {
                loading.value = true;
                ...
                error.value.search = err;
                core.Logger.error(`useReview/${id}/search`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const loadCustomerReviews = async () => {
            core.Logger.debug(`useReview/${id}/loadCustomerReviews`);
            try {
                loading.value = true;
                ...
                core.Logger.error(`useReview/${id}/loadCustomerReviews`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const loadReviewMetadata = async (params) => {
            core.Logger.debug(`useReview/${id}/loadReviewMetadata`);
            try {
                loading.value = true;
                ...
                core.Logger.error(`useReview/${id}/loadReviewMetadata`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const addReview = async (params) => {
            core.Logger.debug(`useReview/${id}/addReview`, params);
            try {
                loading.value = true;
                ...
                core.Logger.error(`useReview/${id}/addReview`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        return {
            ...
            loadReviewMetadata,
            loadCustomerReviews,
            metadata: compositionApi.computed(() => metadatas.value),
            reviews: compositionApi.computed(() => reviews.value),
            loading: compositionApi.computed(() => loading.value),
            error: compositionApi.computed(() => error.value),
        };
    };
...
const factoryParams$8 = {
    ...
    searchReviews: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento] search review params input:', JSON.stringify(params, null, 2));
    ...
    addReview: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento] add review params input:', JSON.stringify(params, null, 2));
        const { customQuery, ...input } = params;
    ...
    loadReviewMetadata: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento] load review metadata');
    ...
    loadCustomerReviews: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] load customer review based on:', { params });
...
const factoryParams$7 = {
    ...
    load: async (context, { customQuery }) => {
        var _a, _b;
        core.Logger.debug('[Magento] loadShipping', { customQuery });
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    save: async (context, params) => {
        core.Logger.debug('[Magento] save user shipping address', { params });
        const { id } = context.cart.cart.value;
...
var index$4 = core.useShippingFactory(factoryParams$7);

function useStoreFactory(factoryParams) {
    return function useStore(cacheId = '') {
        const ssrKey = cacheId || 'useStoreFactory';
        // @ts-ignore
        const stores = core.sharedRef([], `${ssrKey}-stores`);
        const loading = core.sharedRef(false, `${ssrKey}-loading`);
        // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle
        const _factoryParams = core.configureFactoryParams(factoryParams);
        const load = async (params) => {
            core.Logger.debug(`useStore/${ssrKey}/load`);
            loading.value = true;
            try {
        ...
        return {
            load,
            change,
            stores: compositionApi.computed(() => stores.value),
            loading: compositionApi.computed(() => loading.value),
        };
    };
}
...
function useUpsellProductsFactory(factoryParams) {
    return function useUpsellProducts(id) {
        const products = core.sharedRef([], `useUpsellProducts-products-${id}`);
        const loading = core.sharedRef(false, `useUpsellProducts-loading-${id}`);
        const error = core.sharedRef({
            search: null,
        ...
        const search = async (searchParams) => {
            core.Logger.debug(`useUpsellProducts/${id}/search`, searchParams);
            try {
                loading.value = true;
                products.value = await _factoryParams.productsSearch(searchParams);
                error.value.search = null;
                ...
                error.value.search = err;
                core.Logger.error(`useUpsellProducts/${id}/search`, err);
            }
            finally {
                loading.value = false;
            }
...
const factoryParams$5 = {
    productsSearch: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento] Find upsell products based on ', { params });
...
const useUrlResolverFactory = (factoryParams) => function useRouter(ssrKey = 'useRouter') {
    ...
    // eslint-disable-next-line consistent-return
    const search = async (params) => {
        core.Logger.debug(`useRouter/${ssrKey}/search`);
        loading.value = true;
        try {
            const data = await _factoryParams.search(params);
            result.value = data;
            ...
            error.value.search = err;
            core.Logger.error(`useRouter/${ssrKey}/search`, err);
        }
        finally {
            loading.value = false;
        }
...
const factoryParams$4 = {
    search: async (context, params) => {
        core.Logger.debug('[Magento] Find information based on URL', { params });
...
const factoryParams$3 = {
    ...
    addAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento]: add billing address', { params });
    ...
    deleteAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] delete billing address', { params });
        const { data } = await context.$magento.api.deleteCustomerAddress(params.address.id);
        core.Logger.debug('[Result]:', { data });
    ...
    updateAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] update billing address', { params });
        const { data } = await context.$magento.api.updateCustomerAddress(transformUserUpdateAddressInput(params));
        core.Logger.debug('[Result]:', { data });
        return (_a = data === null || data === void 0 ? void 0 : data.updateCustomerAddress) !== null && _a !== void 0 ? _a : {};
    },
    ...
    load: async (context, _params) => {
        var _a, _b, _c;
        core.Logger.debug('[Magento] load user address');
    ...
    setDefaultAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] setDefaultAddress');
        const { data } = await context.$magento.api.updateCustomerAddress(transformUserUpdateAddressInput(params));
        core.Logger.debug('[Result]:', { data });
        return (_a = data === null || data === void 0 ? void 0 : data.updateCustomerAddress) !== null && _a !== void 0 ? _a : {};
    },
};
var index$3 = core.useUserBillingFactory(factoryParams$3);
...
const factoryParams$2 = {
    ...
    searchOrders: async (context, params) => {
        var _a, _b, _c, _d;
        core.Logger.debug('[Magento] search user orders', { params });
        if (!((_b = (_a = context.user.user) === null || _a === void 0 ? void 0 : _a.value) === null || _b === void 0 ? void 0 : _b.id)) {
            await context.user.load();
        }
...
var index$2 = core.useUserOrderFactory(factoryParams$2);
...
const factoryParams$1 = {
    ...
    addAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento]: add shipping address', { params });
        const { data } = await context.$magento.api.createCustomerAddress(transformUserCreateAddressInput(params));
        core.Logger.debug('[Result]:', { data });
        return (_a = data === null || data === void 0 ? void 0 : data.createCustomerAddress) !== null && _a !== void 0 ? _a : {};
    },
    deleteAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] delete shipping address', { params });
        const { data } = await context.$magento.api.deleteCustomerAddress(params.address.id);
        return (_a = data === null || data === void 0 ? void 0 : data.deleteCustomerAddress) !== null && _a !== void 0 ? _a : {};
    },
    updateAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] update shipping address', { params });
        const { data } = await context.$magento.api.updateCustomerAddress(transformUserUpdateAddressInput(params));
        return (_a = data === null || data === void 0 ? void 0 : data.updateCustomerAddress) !== null && _a !== void 0 ? _a : {};
    },
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    setDefaultAddress: async (context, params) => {
        var _a;
        core.Logger.debug('[Magento] set default shipping address');
        const { data } = await context.$magento.api.updateCustomerAddress(transformUserUpdateAddressInput(params));
        core.Logger.debug('[Result]:', { data });
        return (_a = data === null || data === void 0 ? void 0 : data.updateCustomerAddress) !== null && _a !== void 0 ? _a : {};
    },
};
var index$1 = core.useUserShippingFactory(factoryParams$1);
...
const useWishlistFactory = (factoryParams) => {
    ...
    const useWishlist = (ssrKey = 'useWishlistFactory') => {
        const loading = core.sharedRef(false, `useWishlist-loading-${ssrKey}`);
        const wishlist = core.sharedRef(null, `useWishlist-wishlist-${ssrKey}`);
        const itemsCount = core.sharedRef(0, `useWishlist-itemsCount-${ssrKey}`);
        const error = core.sharedRef({
            addItem: null,
            removeItem: null,
            load: null,
        ...
        const setWishlist = (newWishlist) => {
            ...
            core.Logger.debug(`useWishlistFactory/${ssrKey}/setWishlist`, newWishlist);
        };
        const addItem = async ({ product, customQuery }) => {
            core.Logger.debug(`useWishlist/${ssrKey}/addItem`, product);
            try {
                loading.value = true;
                ...
                error.value.addItem = err;
                core.Logger.error(`useWishlist/${ssrKey}/addItem`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const removeItem = async ({ product, customQuery }) => {
            core.Logger.debug(`useWishlist/${ssrKey}/removeItem`, product);
            try {
                loading.value = true;
                ...
                error.value.removeItem = err;
                core.Logger.error(`useWishlist/${ssrKey}/removeItem`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const load = async (params) => {
            core.Logger.debug(`useWishlist/${ssrKey}/load`);
            try {
                loading.value = true;
                ...
                error.value.load = err;
                core.Logger.error(`useWishlist/${ssrKey}/load`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const loadItemsCount = async (params) => {
            core.Logger.debug(`useWishlist/${ssrKey}/loadItemsCount`);
            try {
                ...
                core.Logger.error(`useWishlist/${ssrKey}/loadItemsCount`, err);
            }
        };
        const clear = async () => {
            core.Logger.debug(`useWishlist/${ssrKey}/clear`);
            try {
                loading.value = true;
                ...
                error.value.clear = err;
                core.Logger.error(`useWishlist/${ssrKey}/clear`, err);
            }
            finally {
                loading.value = false;
            }
        ...
        const isInWishlist = ({ product }) => {
            core.Logger.debug(`useWishlist/${ssrKey}/isInWishlist`, product);
...
const factoryParams = {
    ...
    load: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento Storefront]: useWishlist.load params->', params);
        const apiState = context.$magento.config.state;
        if (apiState.getCustomerToken()) {
    ...
    loadItemsCount: async (context, params) => {
        var _a, _b;
        core.Logger.debug('[Magento Storefront]: useWishlist.wishlistItemsCount params->', params);
        const apiState = context.$magento.config.state;
        if (apiState.getCustomerToken()) {
    ...
    addItem: async (context, params) => {
        ...
        core.Logger.debug('[Magento Storefront]: useWishlist.addItem params->', params);
        if (!currentWishlist)
        ...
        switch (product.__typename) {
            ...
            case 'ConfigurableProduct':
                ...
                core.Logger.debug('[Result]:', { data: configurableProductData });
            ...
            case 'BundleProduct':
                ...
                core.Logger.debug('[Result]:', { data: bundleProductData });
    ...
    removeItem: async (context, params) => {
        ...
        core.Logger.debug('[Magento Storefront]: useWishlist.removeItem params->', params);
        const itemOnWishlist = findItemOnWishlist(currentWishlist, params.product);
...
/* istanbul ignore file */
core.track('VSFMagento');

exports.addressGetter = addressGetter;
exports.cartGetters = cartGetters$1;
exports.categoryGetters = categoryGetters$1;
exports.checkoutGetters = checkoutGetters$1;
exports.extendScopeContext = extendScopeContext;
exports.facetGetters = facetGetters$1;
exports.forgotPasswordGetters = forgotPasswordGetters$1;
exports.orderGetters = orderGetters$1;
exports.productGetters = productGetters$1;
exports.reviewGetters = reviewGetters$1;
exports.storeConfigGetters = StoreConfigGetters;
exports.storeGetters = storeGetters$1;
exports.useAddresses = index$g;
exports.useBilling = index$f;
exports.useCart = useCart;
exports.useCategory = index$e;
exports.useCategorySearch = useCategorySearch$1;
exports.useConfig = useConfig$1;
exports.useContent = index$d;
exports.useCountrySearch = useCountrySearch$1;
exports.useCurrency = useCurrency$1;
exports.useCustomMutation = index$c;
exports.useCustomQuery = index$b;
exports.useExternalCheckout = index$a;
exports.useFacet = index$9;
exports.useForgotPassword = useForgotPassword$1;
exports.useGetShippingMethods = useGetShippingMethods$1;
exports.useGuestUser = index$8;
exports.useMakeOrder = useMakeOrder$1;
exports.useNewsletter = index$7;
exports.usePaymentProvider = index$6;
exports.useProduct = useProduct$1;
exports.useRelatedProducts = useRelatedProducts$1;
exports.useReview = index$5;
exports.useShipping = index$4;
exports.useShippingProvider = useShippingProvider;
exports.useStore = useStore$1;
exports.useUpsellProducts = useUpsellProducts$1;
exports.useUrlResolver = useUrlResolver$1;
exports.useUser = useUser;
exports.useUserBilling = index$3;
exports.useUserOrder = index$2;
exports.useUserShipping = index$1;
exports.useWishlist = index;
exports.userAddressesGetters = userAddressesGetters$1;
exports.userBillingGetters = userBillingGetters$1;
exports.userGetters = userGetters$1;
exports.userShippingGetters = userShippingGetters$1;
exports.wishlistGetters = wishlistGetters$1;
