var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
import * as React from "react";
function o2s(style) {
    var string = "";
    Object.keys(style).forEach(function (a) {
        string += "".concat(a, ": ").concat(style[a], ";");
    });
    return string;
}
export var useInjectCustomProperties = function (customProperties) {
    var styles = Object.keys(customProperties)
        .filter(function (i) { return customProperties[i].value; })
        .reduce(function (o, key) {
        var _a;
        return (__assign(__assign({}, o), (_a = {}, _a["--".concat(key)] = customProperties[key].value, _a)));
    }, {});
    var previewRef = React.useRef();
    React.useLayoutEffect(function () {
        var _a;
        var iframe = document.querySelector("#storybook-preview-iframe");
        if (iframe) {
            previewRef.current = (_a = iframe === null || iframe === void 0 ? void 0 : iframe.contentWindow) === null || _a === void 0 ? void 0 : _a.document;
        }
        else if (document) {
            previewRef.current = document;
        }
    }, [customProperties]);
    React.useLayoutEffect(function () {
        var _a;
        var stringifiedStyles = o2s(styles);
        (_a = previewRef === null || previewRef === void 0 ? void 0 : previewRef.current) === null || _a === void 0 ? void 0 : _a.body.setAttribute("style", stringifiedStyles);
    }, [customProperties, styles]);
};
