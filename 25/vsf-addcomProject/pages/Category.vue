  ...
  <div id="category">
    ...
    <div class="sf-breadcrumbs__breadcrumb" v-if="routeData && routeData.relative_url">
      <template v-for="head in routeData.relative_url.split('.').slice(0, 1)">
        <router-link to="/addcom"> Home &nbsp;&nbsp;&nbsp;| &nbsp;&nbsp;&nbsp; </router-link> 
        <router-link :to="'/addcom/c/'+head+'.html'"> {{ head.charAt(0).toUpperCase() + head.slice(1).replaceAll('/', '&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;') }} &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;  </router-link> 
      </template>
    </div>
        ...
        <div v-if="!isProductsLoading" class="products">
          ...
          >
            <SfProductCardHorizontal
              ...
              :link="
                localePath(
                  `addcom/p/${productGetters.getProductSku(
                    product
                  )}${productGetters.getSlug(product, product.categories[0])}`
                )
              "
